...
- Momentum: 7 bar, ATR_rel 0.12% (szigorított), NSDQ100 a teljes cash sessionben (13:30–20:00Z).
- ÚJ minőség-kapu: minimális EMA9–EMA21 relatív gap (asset-függő).
...
Robusztussági fixek:
- ATR(1h) és ATR(5m) NaN-védelem.
- ÚJ: “félkész” 5m bar szűrése — ha az utolsó 5m gyertya kora < UNFINISHED_5M_AGE_SEC, levágjuk az utolsót.
"""
...
LEVERAGE = {"SOL": 3.0, "NSDQ100": 3.0, "GOLD_CFD": 2.0, "BNB": 3.0, "USOIL": 2.0}
MAX_RISK_PCT = 1.8
...
# --- Fib toleranciák ---
FIB_TOL_FRAC = 0.025
FIB_TOL_ABS_ATR1H_MULT = 1.0

# --- ATR küszöbök (REL) — asset-függő ---
ATR_LOW_TH_ASSET = {"default": 0.0007, "SOL": 0.0008, "BNB": 0.0008, "NSDQ100": 0.0007, "GOLD_CFD": 0.0007, "USOIL": 0.0007}
def atr_threshold(asset: str) -> float: return ATR_LOW_TH_ASSET.get(asset, ATR_LOW_TH_ASSET["default"])

# --- Kereskedési/egz. küszöbök (RR/TP) ---
MIN_R, TP1_R, TP2_R = 2.0, 2.0, 3.0

TP_MIN_PCT = {"default": 0.0030, "GOLD_CFD": 0.0015, "USOIL": 0.0020, "NSDQ100": 0.0020, "SOL": 0.0040, "BNB": 0.0040}
TP_MIN_ABS = {"default": 0.50, "GOLD_CFD": 3.0, "USOIL": 0.08, "NSDQ100": 0.80, "SOL": 0.80, "BNB": 0.50}
COST_ROUND_PCT_ASSET = {"default": 0.0015, "GOLD_CFD": 0.0008, "USOIL": 0.0010, "NSDQ100": 0.0007, "SOL": 0.0020, "BNB": 0.0020}
COST_MULT_BASE, COST_MULT_HIGH_VOL, HIGH_VOL_TH, ATR5_MIN_MULT = 2.0, 1.7, 0.0020, 0.5

# --- Momentum override eszközök + minőségkapuk ---
ENABLE_MOMENTUM_ASSETS = {"SOL", "BNB", "NSDQ100"}
MOMENTUM_BARS = int(os.getenv("MOMENTUM_BARS", "7"))
MOMENTUM_ATR_REL = float(os.getenv("MOMENTUM_ATR_REL", "0.0012"))  # 0.12%
MOMENTUM_BOS_LB  = 15

# min. EMA-gap (relatív az árhoz)
MOMENTUM_EMA_GAP_REL = {
    "default": 0.00030,  # 0.030%
    "SOL":     0.00050,  # 0.050%
    "BNB":     0.00040,  # 0.040%
    "NSDQ100": 0.00025,  # 0.025%
}
def ema_gap_threshold(asset: str) -> float: return MOMENTUM_EMA_GAP_REL.get(asset, MOMENTUM_EMA_GAP_REL["default"])

def momentum_time_ok(asset: str, now_utc: Optional[datetime] = None) -> bool:
    """Külön időablak nélkül; NSDQ100-nál a session_ok (13:30–20:00Z) kapuz."""
    return asset in {"SOL", "BNB", "NSDQ100"}

# --- Rezsim és session ---
EMA_SLOPE_PERIOD, EMA_SLOPE_LOOKBACK = 21, 3
EMA_SLOPE_TH_ASSET = {"default": 0.0010, "GOLD_CFD": 0.0008, "USOIL": 0.0008}
def ema_slope_threshold(asset: str) -> float: return EMA_SLOPE_TH_ASSET.get(asset, EMA_SLOPE_TH_ASSET["default"])

SESSIONS_UTC: Dict[str, Optional[List[Tuple[int,int,int,int]]]] = {
    "SOL": None, "BNB": None, "NSDQ100": [(13,30, 20,0)], "GOLD_CFD": None, "USOIL": None,
}

# -------------------------- segédek -----------------------------------
def nowiso() -> str: return datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def in_any_window_utc(windows: Optional[List[Tuple[int,int,int,int]]], h: int, m: int) -> bool:
    ...
    return False
def session_ok(asset: str) -> bool: return in_any_window_utc(SESSIONS_UTC.get(asset), *now_utctime_hm())
...
def as_df_klines(raw: Any) -> pd.DataFrame:
    if not raw: return pd.DataFrame(columns=["open","high","low","close"])
    arr = raw if isinstance(raw, list) else (raw.get("values") or [])
    rows: List[Dict[str, Any]] = []
    for x in arr:
        try:
            if "datetime" in x:
                dt = pd.to_datetime(x["datetime"], utc=True); o,h,l,c = float(x["open"]),float(x["high"]),float(x["low"]),float(x["close"])
                v = float(x.get("volume", 0.0) or 0.0)
            elif "t" in x:
                dt = pd.to_datetime(x["t"], utc=True); o,h,l,c = float(x["o"]),float(x["h"]),float(x["l"]),float(x["c"])
...
def find_swings(df: pd.DataFrame, lb: int = 2) -> pd.DataFrame:
    ...
    out = df.copy(); out["swing_hi"] = swing_hi.fillna(False); out["swing_lo"] = swing_lo.fillna(False)
    return out
...
def last_swing_levels(df: pd.DataFrame) -> Tuple[Optional[float], Optional[float]]:
    ...
    hi = df[df["swing_hi"]].tail(1)["high"].values; lo = df[df["swing_lo"]].tail(1)["low"].values
...
def detect_sweep(df: pd.DataFrame, lookback: int = 24) -> Dict[str, bool]:
    ...
    ref = df.iloc[-(lookback+1):-1]; last = df.iloc[-1]
...
def detect_bos(df: pd.DataFrame, direction: str) -> bool:
    ...
    sw = find_swings(df, lb=2); hi, lo = last_swing_levels(sw.iloc[:-1])
    if direction == "long" and hi is not None:  return sw["high"].iloc[-1] > hi
    if direction == "short" and lo is not None: return sw["low"].iloc[-1] < lo
    return False
...
def broke_structure(df: pd.DataFrame, direction: str, lookback: int = MOMENTUM_BOS_LB) -> bool:
    ...
    ref = df.iloc[-(lookback+1):-1]; last = df.iloc[-1]
    if direction == "long":  return last["high"] > ref["high"].max()
    if direction == "short": return last["low"]  < ref["low"].min()
    return False

def fib_zone_ok(move_hi, move_lo, price_now, low=0.618, high=0.886, tol_abs=0.0, tol_frac=0.02) -> bool:
    if move_hi is None or move_lo is None or move_hi == move_lo: return False
    length = move_hi - move_lo
    z1_long  = move_lo + low  * length; z2_long  = move_lo + high * length
    z1_short = move_hi - high * length; z2_short = move_hi - low  * length
...
def bias_from_emas(df: pd.DataFrame) -> str:
    ...
    c = df["close"]; e9,e21,e50,e200 = ema(c,9).iloc[-1], ema(c,21).iloc[-1], ema(c,50).iloc[-1], ema(c,200).iloc[-1]
    last = c.iloc[-1]
    if last > e200 and e50 > e200 and e9 > e21:  return "long"
    if last < e200 and e50 < e200 and e9 < e21:  return "short"
    return "neutral"

def ema_slope_ok(df_1h: pd.DataFrame, period: int = EMA_SLOPE_PERIOD, lookback: int = EMA_SLOPE_LOOKBACK, th: float = 0.0010) -> Tuple[bool, float]:
...
def drop_unfinished_last_5m(df: pd.DataFrame, min_age_sec: int = UNFINISHED_5M_AGE_SEC) -> pd.DataFrame:
    ...
    if age is not None and age < min_age_sec and len(df) >= 2: return df.iloc[:-1].copy()
    return df.copy()

# ------------------------------ elemzés egy eszközre ---------------------------
def analyze(asset: str) -> Dict[str, Any]:
    outdir = os.path.join(PUBLIC_DIR, asset); os.makedirs(outdir, exist_ok=True)
    ...
    if (spot_price is None) or k5m.empty or k1h.empty or k4h.empty:
        msg = {"asset": asset, "ok": False, "retrieved_at_utc": nowiso(), "source": "Twelve Data (lokális JSON)",
               "spot": {"price": spot_price, "utc": spot_utc}, "signal": "no entry", "probability": 0,
               "entry": None, "sl": None, "tp1": None, "tp2": None, "rr": None,
               "leverage": LEVERAGE.get(asset, 2.0), "reasons": ["Insufficient data (spot/k5m/k1h/k4h)"]}
        save_json(os.path.join(outdir, "signal.json"), msg); return msg
    ...
    if k5m_closed.empty or k1h_closed.empty or k4h_closed.empty:
        msg = {"asset": asset, "ok": False, "retrieved_at_utc": nowiso(), "source": "Twelve Data (lokális JSON)",
               "spot": {"price": float(spot_price), "utc": spot_utc}, "signal": "no entry", "probability": 0,
               "reasons": ["Insufficient closed bars after unfinished-5m filter"]}
        save_json(os.path.join(outdir, "signal.json"), msg); return msg

    display_spot = float(spot_price)
    price_for_calc = float(k5m_closed["close"].iloc[-1])
    ...
    bos_core = (trend_bias=="long" and (bos5m_long or bos_struct_long)) or (trend_bias=="short" and (bos5m_short or bos_struct_short))

    # 5) ATR szűrő (relatív)
    atr5_series = atr(k5m_closed); atr5 = float(atr5_series.iloc[-1]) if len(atr5_series) else float("nan")
    ...
    # 6) Fib zóna (1h swingek)
    k1h_sw = find_swings(k1h_closed, lb=2); move_hi, move_lo = last_swing_levels(k1h_sw)
    atr1h_series = atr(k1h_closed); atr1h = float(atr1h_series.iloc[-1]) if len(atr1h_series) else 0.0
    if not np.isfinite(atr1h): atr1h = 0.0
    fib_ok = fib_zone_ok(move_hi, move_lo, price_for_calc, low=0.618, high=0.886,
                         tol_abs=atr1h * FIB_TOL_ABS_ATR1H_MULT, tol_frac=FIB_TOL_FRAC)
    ...
    conds_core = {"session": bool(session_ok_flag), "regime": bool(regime_ok),
                  "bias": trend_bias in ("long","short"), "bos5m|struct_break": bool(bos_core),
                  "liquidity(fib_zone|sweep)": liquidity_ok, "atr": bool(atr_ok)}
    can_enter_core = (P >= 60) and all(conds_core.values())
    missing_core = [k for k,v in conds_core.items() if not v]

    # --- Momentum (override) + minőség-kapu: EMA-gap
    momentum_used = False; mom_dir: Optional[str] = None; missing_mom: List[str] = []
    if (asset in ENABLE_MOMENTUM_ASSETS) and momentum_time_ok(asset):
        e9_5 = ema(k5m_closed["close"], 9); e21_5 = ema(k5m_closed["close"], 21)
        bear = (e9_5 < e21_5).tail(MOMENTUM_BARS).all(); bull = (e9_5 > e21_5).tail(MOMENTUM_BARS).all()

        gap_rel = (e9_5 - e21_5).abs() / k5m_closed["close"].replace(0, np.nan)
        gap_ok  = float(gap_rel.tail(MOMENTUM_BARS).min()) >= ema_gap_threshold(asset)
        ...
        if mom_dir is None:
            ...
            if not gap_ok:          missing_mom.append("momentum_gap")
    ...
    # 8) Döntés + szintek
    decision = "no entry"; entry = sl = tp1 = tp2 = rr = None
    lev = LEVERAGE.get(asset, 2.0); mode = "core"; missing = list(missing_core)

    def nsdq_cash_session_now() -> bool: return asset == "NSDQ100" and session_ok("NSDQ100")
    ...
    def compute_levels(decision_side: str):
        ...
        k5_sw = find_swings(k5m_closed, lb=2); hi5, lo5 = last_swing_levels(k5_sw)
        ...
        if decision_side == "buy":
            ...
            rr  = (tp2 - entry) / risk; tp1_dist = tp1 - entry
            ...
            tp1 = entry - TP1_R * risk; tp2 = entry - TP2_R * risk
            rr  = (entry - tp2) / risk; tp1_dist = entry - tp1
        ...
        if asset == "NSDQ100" and nsdq_cash_session_now(): tp_min_pct_asset = min(tp_min_pct_asset, 0.0010)
    ...
    if can_enter_core:
        decision = "buy" if trend_bias=="long" else "sell"; mode = "core"
        if not compute_levels(decision): decision = "no entry"
    else:
        if mom_dir is not None:
            mode = "momentum"; missing = []; momentum_used = True; decision = mom_dir
            if not compute_levels(decision): decision = "no entry"
