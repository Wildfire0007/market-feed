...
def ema_cross_recent(short: pd.Series, long: pd.Series, bars: int = MOMENTUM_BARS, direction: str = "long") -> bool:
    if short.empty or long.empty or len(short) < bars + 1 or len(long) < bars + 1:
        return False
    short = short.dropna()
    long = long.dropna()
    if len(short) < bars + 1 or len(long) < bars + 1:
        return False
...
def analyze(asset: str) -> Dict[str, Any]:
    ...
    if spot_latency is not None and spot_latency > freshness_limit:
        ...
        save_json(os.path.join(outdir, "signal.json"), payload)
        return payload
