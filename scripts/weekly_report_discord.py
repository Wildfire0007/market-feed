...
def _pnl_week(config,journal, start, end):
    total=0.0; losses=0
    cur=start
    while cur <= end:
        m=compute_daily_labeled_pnl(config,journal,now=min(cur+timedelta(days=1)-timedelta(seconds=1),end))
        total += float(m.get('realized_pnl_usd') or 0.0); losses += int(m.get('losing_trades') or 0); cur += timedelta(days=1)
    return total, losses
def build_embed(now=None):
    ...
    iso,start,end=win; journal=PUBLIC/'journal'/'trade_journal.csv'; rows=_journal_rows(journal,start,end); labeled=[r for r in rows if str(r.get('validation_outcome') or '').strip()]
    assets=Counter(str(r.get('asset') or 'N/A').upper() for r in labeled); hits=sum(1 for r in labeled if str(r.get('validation_outcome') or '').strip().lower() in {'tp_hit','tp1_closed','tp1','tp1_hit','tp2','target','win'} or _float(r.get('validation_rr'))>0)
    asset_txt=', '.join(f'{a}: {n}' for a,n in assets.items()) or 'nincs adat'; n=len(labeled)
    line1=f"Címkézett ügyletek: {n} (eszközönként: {asset_txt})" + (" — nincs adat" if not n else "")
    if n:
        lo,hi=_wilson_interval(hits,n); line2=f"TP1-találat SL előtt: {hits/n*100:.0f}% — 95% Wilson CI: [{lo*100:.0f}%, {hi*100:.0f}%]"
    else: line2="TP1-találat SL előtt: még nincs értékelhető ügylet — a konfidenciaintervallum N≥1-től számolható"
    pnl,losses=_pnl_week(load(ROOT/'config'/'analysis_settings.json'),journal,start,end); line3=f"Realizált heti PnL: ${pnl:.2f} (vesztes ügyletek: {losses})"
