# TD Full Pipeline (5m) audit snapshot — 2025-11-09 10:30 UTC
...
- Pipeline timing indicates ~74 s lag between trading and analysis; no explicit cache invalidation for stale spot/klines means weekend runs load weeks-old spot data, inflating spread guards. A forced realtime snapshot from 2025-10-25 persists in `public/BTCUSD/spot_realtime.json` and produced a 90× ATR spread ratio during the 2025-11-09 10:02 UTC run.
...
- `compute_ofi_zscore` cannot operate on Twelve Data 1m candles because they lack volume columns, so OFI confirmation never materialises; combined with the stale realtime spread this eliminates every BTC candidate.
- Spread gate divides stale forced realtime spot (2025-10-25) by fresh 5m closes, yielding 98×ATR ratios and hard blocks before the new guard was introduced in this patch.
...
## JSON/state hygiene
- Prune or age-out stale `spot_realtime.json` payloads older than 5 minutes; missing bid/ask should nullify `spread_ratio` instead of reusing forced price snapshots. Add unit tests that freeze realtime decisions to prevent regressions (see `tests/test_spread_gate.py`).
