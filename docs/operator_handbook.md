...
1. **Trigger-boncolás** (telemetria N≥50 armed-sor után): melyik részfeltétel bukik a leggyakrabban nem-tüzelő armed futásokon? Ha egyetlen feltétel felel a bukások >60%-áért ÉS a kihagyott jelöltek árnyék-kimenete pozitív várható értékű → az a feltétel kalibrálandó (egyetlen paraméter, egyszerre). **Árnyék-kimenet definíció (fagyasztva: 2026-07-26):** epizód = geometriás armed-sorok klaszterezve (azonos eszköz, ≤30 perc rés, entry-drift ≤0,5R); kontrafaktuális: piaci belépés az első armed-pillanat spotján, SL = terv-stop_loss, TP = spot ± 2×|spot−SL|; kiértékelés végleges 5m gyertyákon (szükség esetén commit-bányászattal), azonos-napi fedés-őrrel (első gyertya ≤10 percen belül), SL-és-TP azonos gyertyában → konzervatívan SL, session-zárásnál (16:25 UTC) mark-to-open. **Első futás (2026-07-26, W30, N=32 epizód):** ΣR=+0,66, átlag +0,02R/epizód, kimenet-mix 9 TP / 19 SL / 4 mark → a pozitív-EV feltétel NEM teljesül, a price_trigger nem kalibrálandó; újrafuttatás N≥60 epizódnál vagy dokumentált rezsimváltásnál.
...
4. **Korai exit / MAE-MFE** (N≥30): a vesztesek ≥60%-ánál van 0,5R-es korai ellenjel, ami a nyertesek <20%-ánál? → órán belüli fordulat-trigger tervezhető. **Stagnálás-alkritérium (előjegyezve: 2026-07-18, időalapú változat):** „kifulladt" címke = max_favorable_rr < 0,5R ÉS tartási idő ≥ 120 perc (forrás: trade_ledger.csv + labeler-kimenet — retenció-független; csak érvényes, nem-expired sorok: tp1_closed vs stopped). Ha N≥30 ledger-trade-nél a címke a vesztesek ≥60%-ánál ÉS a nyertesek <25%-ánál fordul elő → time-stop tervezhető (pl. 120 perc után MFE < 0,5R → zárás), egyetlen változtatásként, a mérési ablak után. EV-indok: RR 2,0 mellett a korai zárás a vesztesen ~0,9R-t ment, a nyertesen ~2R-t dob el → ≥2,2–2,5× arány szükséges; a 60/25 pár (2,4×) fedezi.
...
- **Fájl-életciklus térkép és új-artifact checklist (rögzítve: 2026-07-30):** a pipeline a fájlokat rétegeken át mozgatja: (1) Trading.py a `data/` stagingbe ír → (2) `update_public.py` töröl-és-másol a `public/`-ba (a data-ban létező alkönyvtárakat cseréli) → (3) a notify job friss checkout után `rm -rf public` + artifact-letöltés → (4) `preserve_public_append_logs.py` menti és összefésüli a listázott append-naplókat → (5) `git add public` commitol. MINDEN új napló/artifact bevezetésekor kötelező öt kérdés: melyik jobban íródik? · túléli-e az update_public cserét? · benne van-e az artifactban? · szerepel-e a preserve-listában? · commitolja-e a záró lépés? Élő elfogadás mindig futásokon ÁTÍVELŐ halmozódással mérendő, nem egyszeri jelenléttel.
- **USOIL-felfüggesztés és visszavételi eljárás (2026-08-01):** a USOIL feed-integritási karantén alatt (revíziós járvány, fantom-SL) — nincs elemzés, nincs kártya. Visszavétel legkorábban 2 hét múlva, kizárólag próbahéttel: az eszköz visszakerül a rosterbe, de az operátori felfüggesztés él (minden USOIL-kártya kihagyandó), miközben a revízió-detektor mér; 5 egymást követő, revízió-mentes kereskedési nap után oldható fel az operátori felfüggesztés, egy döntéssel, changelog-gal. A korábbi USOIL-sorok a ledgerben történelemként maradnak.
- **Heti:** vasárnapi rituálé, kiegészítve (2026-07-30): (e) append-napló monotonitás-audit — a public alatti napló-fájlok sorszáma a hét commit-mintáin csak nőhet; csökkenés = törlés-hiba, azonnali kivizsgálással. **Havi:** backup-workflow kézi próbafuttatás; a CF worker és a webhookok szúrópróbája (/heartbeat) — böngészőből, Cloudflare Access-belépéssel (e-mail + kód); elvárás a JSON-ban: ok=true és ageMin a küszöb alatt (hétköznap 30, hétvége/éjjel 150 perc).
