...
def test_default_profile_configuration(monkeypatch):
    ...
    assert baseline["p_score_min"]["default"] == pytest.approx(42.0)
    assert baseline["p_score_min"]["by_asset"]["BTCUSD"] == pytest.approx(42.0)
    assert baseline["p_score_min"]["by_asset"]["EURUSD"] == pytest.approx(44.0)
    assert baseline["p_score_min"]["by_asset"]["XAGUSD"] == pytest.approx(43.0)
    assert baseline["p_score_min"]["by_asset"]["NVDA"] == pytest.approx(39.0)
    assert baseline["p_score_min"]["by_asset"]["USOIL"] == pytest.approx(43.0)
    assert baseline["atr_threshold_multiplier"]["default"] == pytest.approx(0.90)
    assert baseline["atr_threshold_multiplier"]["by_asset"]["BTCUSD"] == pytest.approx(0.90)
    assert baseline["atr_threshold_multiplier"]["by_asset"]["USOIL"] == pytest.approx(0.90)
    ...
    assert suppressed["atr_threshold_multiplier"]["default"] == pytest.approx(1.05)
    assert suppressed["atr_threshold_multiplier"]["by_asset"]["BTCUSD"] == pytest.approx(1.07)
...
def test_suppressed_profile_configuration(monkeypatch):
    ...
    assert profile["p_score_min"]["by_asset"]["USOIL"] == pytest.approx(49.0)
    assert profile["p_score_min"]["by_asset"]["XAGUSD"] == pytest.approx(49.0)
    assert profile["atr_threshold_multiplier"]["default"] == pytest.approx(1.05)
    assert profile["atr_threshold_multiplier"]["by_asset"]["BTCUSD"] == pytest.approx(1.07)
...
def test_relaxed_profile_override(monkeypatch):
    ...
    # RELAXED értékek
    assert profile["p_score_min"]["by_asset"]["GOLD_CFD"] == pytest.approx(41.0)
    assert profile["p_score_min"]["by_asset"]["BTCUSD"] == pytest.approx(40.0)
    assert profile["atr_threshold_multiplier"]["by_asset"]["USOIL"] == pytest.approx(0.72)
    assert profile["atr_threshold_multiplier"]["by_asset"]["BTCUSD"] == pytest.approx(0.72)
    
    # EURUSD override a relaxed profilban
    assert profile["p_score_min"]["by_asset"]["EURUSD"] == pytest.approx(41.0)
...
def test_profile_specific_helpers(monkeypatch):
    ...
    assert baseline.get_spread_max_atr_pct("EURUSD") == pytest.approx(0.48)
    assert baseline.get_spread_max_atr_pct("USOIL") == pytest.approx(0.42)
    assert baseline.get_spread_max_atr_pct("NVDA") == pytest.approx(0.36)
    assert baseline.get_spread_max_atr_pct("BTCUSD") == pytest.approx(0.57)
    assert baseline.get_spread_max_atr_pct("XAGUSD") == pytest.approx(0.37)
    assert baseline.get_fib_tolerance("GOLD_CFD") == pytest.approx(0.004)
    assert baseline.get_fib_tolerance("NVDA") == pytest.approx(0.0045)
    assert baseline.get_fib_tolerance("EURUSD") == pytest.approx(0.006)
    assert baseline.get_fib_tolerance("USOIL") == pytest.approx(0.0055)
    assert baseline.get_fib_tolerance("XAGUSD") == pytest.approx(0.0045)
    ...
    assert suppressed.get_atr_abs_min("EURUSD") == pytest.approx(0.0008)
    assert suppressed.get_spread_max_atr_pct("BTCUSD") == pytest.approx(0.6)
    ...
    assert suppressed.get_fib_tolerance("GOLD_CFD") == pytest.approx(0.005)
    ...
    assert relaxed.get_fib_tolerance("GOLD_CFD") == pytest.approx(0.0045)
...
def test_risk_template_helpers(monkeypatch):
    ...
    assert settings.get_tp_net_min("GOLD_CFD") == pytest.approx(0.7)
    ...
    assert settings.get_spread_max_atr_pct("BTCUSD") == pytest.approx(0.57)
    assert settings.get_spread_max_atr_pct("USOIL") == pytest.approx(0.42)
    assert settings.get_spread_max_atr_pct("NVDA") == pytest.approx(0.36)
    assert settings.get_spread_max_atr_pct("XAGUSD") == pytest.approx(0.37)

    suppressed = _reload_settings(monkeypatch, profile="suppressed")
    assert suppressed.get_tp_min_pct_value("EURUSD") == pytest.approx(0.0018)
    assert suppressed.get_tp_net_min("BTCUSD") == pytest.approx(0.75)
